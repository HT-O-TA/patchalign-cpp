...
CONFIG_PATH = ROOT / "configs/evaluation/data_v2_exploratory_formal_inference_v0_1.json"
...
def test_exploratory_inference_contract_is_frozen() -> None:
    config = load_config()
    ...
    assert config["source_training"]["git_commit"] == (
        "53329624ddc2ce764b8b7acbf133d6e12682be16"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("source_training", "adapter_sha256"), "sha256:" + "0" * 64),
        (("model", "local_path"), "/tmp/other-model"),
...
def test_exploratory_inference_contract_rejects_drift(
...
def make_training_artifact(tmp_path: Path) -> dict:
    ...
    weights.write_bytes(b"frozen-exploratory-adapter")
...
def test_training_artifact_binding_detects_adapter_tampering(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="exploratory adapter weights hash mismatch"):
