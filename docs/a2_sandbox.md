...
这只关闭 A2 的安全执行与 70 条真实数据 pilot 门禁，不代表 Base/SFT/DPO 模型质量、正式 500 条内部评测集或训练数据已经完成。
...
A2 安全执行与真实 70 条评分闭环已关闭，允许进入后续 Base/外部强基线推理准备。GPU 模型推理仍需另行排队；本次 A2 数据构造、资格和重放均不需要 GPU。RunBugRun/CodeNet 的发布许可审计、正式 500 条内部评测集、模型质量指标和训练仍未完成，不得由本次结果外推。
