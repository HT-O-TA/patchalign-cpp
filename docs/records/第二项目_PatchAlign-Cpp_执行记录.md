...
## 57. GitHub 固定内容终态与 CommitPack 单分片激活
...
## 62. BeetleBox 固定存储 Schema v1.2

v1.1 Job `97505` 在 `gpu25` 以 `COMPLETED 0:0` 用时 38 秒，完成 `417 passed`，但
固定文件中精确 `C++` 为 0/0。一次只读 `language` 列聚合确认实际存储值为小写
`c++`，train/test 实际数为 3,317/3,865；另查三条非正文样本确认 URL、SHA、
updated_files 与时间字段编码可由现有审计器解析。
