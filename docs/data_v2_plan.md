...
v1.1 Job `96412` 实查前 18 条后仍为 0：6 条文件数越界、3 条行数越界、7 条无同仓库显式 issue、1 条 stars 回落、1 条许可证不在 allowlist。全部 48 条搜索结果覆盖 34 个仓库，剩余结果仍有供给检查价值；v1.2 因此按创建时间倒序、在详情请求前按仓库去重，最多再查 12 个仓库，最坏 36 次 core 请求。
