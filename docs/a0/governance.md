...
真实性与污染声明状态：Draft，版本 `0.1.0`
许可证与发布策略状态：**Accepted for A0**，版本 `0.1.0`
发布策略决策日期：2026-09-01

本文合并真实性、污染、归因、许可证和产物发布边界；各分项状态独立保留，合并不改变已接受决策。
...
当前仅完成 G0 真实模型 smoke 和 A0 合成评分/门禁测试；尚未构建正式数据、运行 Base 冻结基线、完成 SFT/DPO、获得 Pass@1 提升或验证安全沙箱。低等级证据不得包装成高等级结论。
