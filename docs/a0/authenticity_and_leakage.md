...
详细规则见 [许可证与产物发布策略](publication_policy.md)。当前边界为：

- 正式 SFT/DPO adapter 可以在基础模型、训练数据、敏感信息和模型卡审计后公开；
- 中间 checkpoint、optimizer state 和 G0 smoke adapter 默认不公开；
- 未完成逐来源许可审计前不公开原始或重打包训练数据；
- 完整预测需要通过来源代码许可、敏感信息和漏洞披露检查；
- 原始失败日志不公开，但失败样本仍必须进入统计分母；
- 公开报告使用稳定 artifact ID 或脱敏占位符，不暴露祝融用户名、节点和绝对内部路径。
