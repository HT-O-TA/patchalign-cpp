...
- 外部终态中 M0/M1-R2 都只有 1/176 Pass；R2 把 parse/apply/build 从 94/24/17 提高到 174/72/55，但没有提高端到端 Pass。外部门禁通过只表示未超过 2pp 退化上限，不等于性能提升。
- A4 首次 CPU 作业发现“27 个 file-window 备用候选”与“每题至少 5 条测试”在固定输入上不可同时满足。选择保留质量门槛和最终 8 条目标，只把备用池修正为真实可行的 26，并补充 ADR、真实数据审计和失败目录归档。

面试要点：晋级条件是合取而非择优展示。即便旧测试集通过，只要独立确认集失败，就必须停止；外部评测仍应跑完以形成完整证据，而不是用失败结果反向调参。

### A4：负责人授权的可执行偏好数据研究

- 在确认集门禁失败、`a4_ready=false` 的事实不变前提下，通过 ADR 明确区分“正式晋级”和“负责人授权的 exploratory 失败模式研究”。
- 只从冻结 train 的 RunBugRun C++ 构造数据。CPU 双资格冻结 256 function + 8 file-window；M1-R2 单 GPU 为每例生成 4 个候选，共 1,056 个，全部生成成功且 seed replay 稳定。
- 在执行结果出现前冻结排序：只比较同题候选到达的执行阶段，同终态只区分 timeout；不用 gold 相似度、patch 长度或事后阈值。每例最多一对，无严格差异就放弃。
- 将评分拆为 264 个 CPU-only 原子 checkpoint，评分训练数据与执行 audit 分离；训练文件不携带 gold、测试或终态标签。
- 最终 1,056 个候选得到 123 个完整 Pass 和 11 个 timeout；77/264 个案例至少有一个成功候选，形成 182 对偏好数据，其中 75 对 chosen 为 success，82 例因无严格差异主动放弃。
- 某计算节点让 preflight、数组和聚合反复出现零日志；通过同提交跨节点对照、保留 Job 重排和精确排除节点恢复。另一个长尾案例的三个候选分别在 18 个 public tests 上全部 timeout，用时 54 分钟，仍保留在分母中。
...
9. 冻结候选配额与真实最小测试覆盖联合不可满足：不降低 5-test 门槛或最终组成，只修正备用池并把真实可行域审计加入数据前置检查。
...
7. 最后说明新确认集失败、Defects4C 最终 Pass 无提升，因此 A4 不可正式晋级；当前 exploratory A4 是负责人授权的失败模式研究，不能称为门禁通过。

## 可以诚实声称与不能声称的内容

可以声称：已完成可复现的数据、训练、生成、沙箱评分和比较流水线；SFT 显著改善格式遵循，并在冻结 function holdout 上达到预注册的主提升要求；超时退化得到独立复现和逐例根因。

目前不能声称：完整 SFT promotion gate 已通过；模型已在新的未见确认集上泛化；M1-R2 在 Defects4C 上提高了最终 Pass；A4 已正式晋级或 DPO 已开始。可以声称 pre-A4 账本已经闭环，负责人授权的 exploratory A4 已完成 264 条数据、1,056 个候选真实评分和 182 对保守偏好数据构造；其中 123 个候选完整通过、11 个 timeout。A5 仍未开始。
