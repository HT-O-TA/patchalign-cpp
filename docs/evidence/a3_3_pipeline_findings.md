...
### 13. A4 多候选执行产生可用偏好信号，也暴露 timeout 长尾

- 证据：264 个 train-only 可执行案例各生成 4 个候选。1,056 个候选 parse/apply/compile/Pass 为 1,055/805/780/123，timeout 11；77 个案例至少有一个 success。按预先冻结的执行阶段与 timeout 排序形成 182 对，82 例因最高/最低键相同而不配对；75 对 chosen 为 success，另外 107 对只提供较弱的阶段质量信号。
- 调度归因：preflight、原数组和聚合在 `gpu16` 上均出现零日志或零 checkpoint；相同提交排除该节点后，preflight 18 秒、聚合 2 秒完成，替换数组立即增长。因此无效运行归因于节点，而不是评分代码或候选。
- 长尾：绝大多数案例为秒级，但 index 50 的四个候选均在 18 个 public tests 上失败，其中三个候选在 18 项上全部 timeout，完整记录使任务用时 54 分 6 秒。项目未改成 fail-fast，保留 11 个 timeout 及其 outcomes；案例级 checkpoint 让一个拖尾任务不阻塞其他 263 例。
- 方法学意义：多次采样在已筛选 train-only 分布上得到候选成功率 11.65% 和经验 Pass@4 29.17%，说明执行选择有信息量，但不能与未见集 greedy Pass@1 比较。偏好数据产量也不能只报 182 对：必须区分 75 对 success 对比和 107 对非 success 阶段对比，并在 DPO 前评估弱标签噪声与 timeout 风险。
- 关键身份：scores `c218cd58...cee2`，preferences `5e6b56e4...78bf`，pair audit `bcaf461d...b6ad2`，summary `302e7a9a...fc8`，run manifest `03c61f0e...7cbe`。
...
- A4 偏好对中 success/非 success 信号对后续优化稳定性的影响；
