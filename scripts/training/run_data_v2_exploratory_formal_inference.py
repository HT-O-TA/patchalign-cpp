"""Segmented deterministic formal inference for the exploratory adapter."""
...
from scripts.training.data_v2_exploratory_inference_common import (
    validate_config,
...
def main() -> None:
    ...
    parser.add_argument("--role", choices=("dv2",), default="dv2")
    ...
    require(
        ...
        "exploratory inference requires a clean worktree",
    )
    require(args.config.is_file(), "exploratory inference config missing")
    require(args.preflight.is_file(), "exploratory inference preflight missing")
    ...
    require(preflight["version"] == "data-v2-exploratory-formal-inference-preflight-v0.1", "wrong inference preflight version")
    require(preflight["status"] == "passed", "exploratory inference preflight did not pass")
    ...
    expected_state = {
        "version": "data-v2-exploratory-formal-inference-state-v0.1",
        "role": args.role,
        "git_commit": commit,
    ...
    if state_path.exists():
        ...
        for key, value in expected_state.items():
            require(state[key] == value, f"exploratory inference resume mismatch: {key}")
    else:
    ...
    summary = {
        "version": "data-v2-exploratory-formal-generation-v0.1",
        "role": args.role,
        "cases": len(records),
