"""Checkpointed M1-R2 continuation for Data-v2 exploratory replay."""
...
from scripts.training.data_v2_exploratory_common import (
    validate_config,
    verify_data,
...
def main() -> None:
    ...
    require(
        ...
        "Data-v2 exploratory training requires a clean worktree",
    )
    require(args.environment_lock.is_file(), "environment lock missing")
    require(args.preflight.is_file(), "exploratory preflight report missing")
    preflight = json.loads(args.preflight.read_text(encoding="utf-8"))
    require(preflight["version"] == "data-v2-exploratory-replay-preflight-v0.1", "wrong exploratory preflight version")
    require(preflight["status"] == "passed", "exploratory preflight did not pass")
    require(preflight["git_commit"] == commit, "exploratory preflight commit mismatch")
    require(preflight["config_sha256"] == sha256_file(args.config), "exploratory preflight config mismatch")
    require(
        preflight["environment_sha256"] == sha256_file(args.environment_lock),
        "exploratory preflight environment mismatch",
    )
    require(
    ...
    if completed_marker.exists():
        ...
        require(manifest["git_commit"] == commit, "completed exploratory run commit mismatch")
        require(manifest["config_sha256"] == sha256_file(args.config), "completed exploratory run config mismatch")
    ...
    require(total_steps == 98, "exploratory optimizer-step denominator mismatch")
    ...
    require(bool(trainable), "no trainable exploratory adapter parameters")
    ...
    state: dict[str, Any] = {
        "version": "data-v2-exploratory-replay-trainer-state-v0.1",
        "git_commit": commit,
    ...
    if resume_dir is not None:
        ...
        for field in (
            ...
            require(loaded[field] == state[field], f"exploratory resume state mismatch: {field}")
    ...
    for epoch_index in range(state["next_epoch"], len(orders)):
        ...
        for start in range(start_offset, len(order), accumulation):
            ...
            for index in group:
                ...
                output = model(**batch)
                loss = output.loss
                require(torch.isfinite(loss).item(), "non-finite exploratory training loss")
            ...
            require(torch.isfinite(grad_norm).item(), "non-finite exploratory gradient norm")
            optimizer.step()
            scheduler.step()
    ...
    require(state["completed_steps"] == total_steps, "exploratory completed step mismatch")
    ...
    require(best_adapter.is_file(), "exploratory best adapter missing")
    summary = {
        "version": "data-v2-exploratory-replay-training-result-v0.1",
        "status": "completed",
    ...
    manifest = {
        ...
        "run_id": f"dv2_exploratory_replay_nf4_s{config['seed']}",
        "stage": "sft",
