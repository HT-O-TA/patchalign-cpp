"""Segmented deterministic inference on the frozen qualified Defects4C set."""
...
from scripts.baseline.run_a3_baseline import generate_one, prompt_sha256
...
def main() -> None:
    ...
    require(not subprocess.check_output(["git", "status", "--porcelain"], cwd=repo, text=True).strip(), "external inference requires a clean worktree")
    ...
    require(Path(config["inference"][args.role]) == args.output_dir, "external output directory differs from config")
    preflight = json.loads(args.preflight.read_text(encoding="utf-8"))
    require(preflight["version"] == "a3-defects4c-inference-preflight-v1" and preflight["status"] == "passed", "external inference preflight did not pass")
    require(preflight["git_commit"] == commit, "external preflight commit mismatch")
    require(preflight["config_sha256"] == sha256_file(args.config), "external preflight config mismatch")
    require(preflight["environment_sha256"] == sha256_file(args.environment_lock), "external preflight environment mismatch")
    require(preflight["dataset_manifest_sha256"] == config["dataset"]["manifest_sha256"], "external preflight dataset mismatch")
    ...
    adapter_sha = ADAPTER_SHA if args.role == "m1_r2" else None
    state_path = args.output_dir / "inference-state.json"
    expected_state = {
        "version": "a3-defects4c-inference-state-v1",
        "role": args.role,
        "git_commit": commit,
    ...
    if state_path.exists():
        ...
        for key, value in expected_state.items():
            require(state[key] == value, f"external inference resume mismatch: {key}")
    else:
    ...
    for case, prompt in zip(manifest["cases"], prompts, strict=True):
        ...
        require(token_count == prompt["input_tokens"], f"token count changed: {case['case_id']}")
        prepared.append({"case": case, "prompt": prompt, "rendered": rendered, "input_tokens": token_count})
    prompts_path = args.output_dir / "prompts.jsonl"
    source_prompts = Path(config["dataset"]["root"]) / config["dataset"]["prompts"]
    if prompts_path.exists():
        require(prompts_path.read_bytes() == source_prompts.read_bytes(), "external prompt artifact changed")
    else:
        shutil.copyfile(source_prompts, prompts_path)
    require(sha256_file(prompts_path) == config["dataset"]["prompts_sha256"], "external prompt artifact hash mismatch")

    partial_path = args.output_dir / "predictions.partial.jsonl"
    records = [] if not partial_path.exists() else [json.loads(line) for line in partial_path.read_text(encoding="utf-8").splitlines()]
    require(len(records) <= len(prepared), "too many external partial predictions")
    require([record["sample_id"] for record in records] == [item["case"]["case_id"] for item in prepared[:len(records)]], "external partial prediction order mismatch")
    ...
    for index in range(len(records), len(prepared)):
        item = prepared[index]
        case, prompt = item["case"], item["prompt"]
        try:
            generated = generate_one(model, tokenizer, torch, item["rendered"], config["generation"])
            ...
            generated = {"raw_text": "", "input_tokens": item["input_tokens"], "output_tokens": 0, "latency_seconds": 0.0, "max_gpu_memory_bytes": int(torch.cuda.max_memory_allocated())}
            raw_text, extracted_patch, status, error = "", None, "oom", f"{type(exc).__name__}: {exc}"
        except Exception as exc:
            generated = {"raw_text": "", "input_tokens": item["input_tokens"], "output_tokens": 0, "latency_seconds": 0.0, "max_gpu_memory_bytes": int(torch.cuda.max_memory_allocated())}
            raw_text, extracted_patch, status, error = "", None, "generation_failed", f"{type(exc).__name__}: {exc}"
        record = {
            "schema_version": "0.1.0",
            "run_id": run_id,
            "sample_id": case["case_id"],
            "model": {"model_id": config["model"]["model_id"], "revision": config["model"]["revision"], "config_sha256": config["model"]["config_sha256"], "adapter_sha256": adapter_sha},
            "prompt_version": config["prompt_version"],
            "prompt_sha256": prompt_sha256(prompt["prompt_text"]),
            "seed": config["seed"],
            ...
            "max_gpu_memory_bytes": int(generated["max_gpu_memory_bytes"]),
        }
        ...
        print(json.dumps({"role": args.role, "index": index + 1, "total": len(prepared), "case": case["case_id"], "status": status}, sort_keys=True), flush=True)
    ...
    probes = []
    for record, item in zip(records[:3], prepared[:3], strict=True):
        require(record["status"] == "ok", f"probe source failed: {record['sample_id']}")
        replay = generate_one(model, tokenizer, torch, item["rendered"], config["generation"])
    ...
    require(not predictions_path.exists(), "refusing to overwrite external predictions")
    partial_path.rename(predictions_path)
    write_json(args.output_dir / "determinism-probe.json", probes)
    summary = {
        "version": "a3-defects4c-generation-v1",
        "role": args.role,
        "cases": len(records),
        ...
        "determinism_probe_stable": all(item["stable"] for item in probes),
    }
    ...
    run_manifest = {
        ...
        "notes": f"role={args.role}; quantization=nf4; prompts={sha256_file(prompts_path)}; preflight={sha256_file(args.preflight)}",
    }
